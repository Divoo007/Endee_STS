...
#    hinging rigidly at one joint -- the base-bent version read as a stiff straight stick
#    jutting off the hand (unnatural from the side). The axis was tuned from the earlier
#    forward-aim [0.7,-0.15,0.7] (twisted ~53deg toward the camera, then ~20deg toward the
#    avatar's RIGHT); the switch to a normal curl is the "make it feel natural" pass. Uses a
#    raw axis, no named axis aims it -- tuned empirically (PoseLab). (Only INDEX base-bends.)
...
# Fist positions below are POST-shoulder-roll (each ~+3cm up, ~+3.7cm forward vs the
# old resting-on-palm arc); the wrist prefixes were re-solved to match (see _help_key).
...
# GEOMETRY (Probe/PoseLab-solved): RightUpperArm [RIGHT -82, UP 38] (fixed), RightLowerArm
# [RIGHT -98, UP 78 near / 62 out], RightWrist roll UP -60 (the forearm twist that turns
# the palm to face the side of the face). The upper-arm UP yaw was lowered 46->38 to shift
# the whole hand a bit more to the LEFT (lateral), moving the fingertips off the front
# hairline so they don't intersect the hair. NEAR keeps a SMALL GAP off the temple (a flat
# hand pressed flush to the head clips into it -- the fingertips sit just OFF the face). The
# tap range was WIDENED (near 78 / out 62, ~16 deg, was 77/71) for a more visible in/out,
# and the hand does the away-and-back tap TWICE (out at t 0.34 and 0.62).
# HANDSHAPE: the four fingers carry a SLIGHT base-knuckle bend ("_base_bend": True, curl 0.3
# -> ~30 deg at the base only, straight beyond) so they angle gently toward the temple --
# NOT dead-flat, NOT the ~90-deg bent-B. Thumb barely curled (0.15).
