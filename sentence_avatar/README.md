...
- Runs in the same Godot project. `SignMainRock.tscn` (the realistic
  Rocketbox avatar) is the project's default scene (see the web app below);
  `render.py` (the emotion demo) explicitly passes `res://scenes/Main.tscn`
  on the `godot` CLI invocation so it isn't affected by that default.
  Writes to `output_signs.mp4` by default (separate from the emotion demo's
  `output.mp4`). The original stylized VRM avatar's scene (`SignMain.tscn`)
  and sign data (`sign_words_vrm_backup.py`) are kept in the project as a
  reference/fallback, not wired into the default build.
