extends Node3D
## Sign-language performer, driven by an already-computed ISL gloss.
##
## Input is a performance request produced upstream by the speech/text -> LLM
## gloss pipeline (see backend/): an ordered gloss where each item carries its
## own {word, emotion, intensity}, plus an overall emotion/intensity. Gloss and
## emotion are NOT derived here -- this script only plays the signs.
##
## Two entry points into the same animation code (_perform_items):
## 1. CLI/video: a JSON config (`-- --config <path>`) carries an explicit
##    "gloss" (word strings OR {word,emotion,intensity} dicts) + emotion; the
##    full per-word keyframe table is passed alongside. Renders and quits.
## 2. Web (index.html): the JS bridge window.godotPerformGloss(json) hands in
##    the performance request; keyframes come from res://data/word_signs.json.
##
...
const DEFAULT_SECONDS_PER_WORD := 2.8  # base pace; scaled per word by emotion intensity
...
var _run_id := 0            # bumped on every perform request so a new
                             # submission cancels whichever one is still playing
var _js_perform_gloss_callback  # kept alive (JavaScriptBridge.create_callback
                           # result is refcounted) for as long as JS can call
                           # window.godotPerformGloss -- a local var would be GC'd
                           # as soon as _ready() returns
...
func _run() -> void:
	...
	# The CLI config carries an explicit "gloss" array (word strings OR
	# {word,emotion,intensity} dicts) plus an optional overall "emotion"/"intensity"
	# and "caption" -- the same performance request the web pipeline hands in, just
	# via a file. There is no sentence-parsing fallback: gloss is computed upstream.
	var gloss_v = config.get("gloss", null)
	if not (gloss_v is Array) or (gloss_v as Array).is_empty():
		push_error("SignDirector: config has no 'gloss' array to perform.")
		get_tree().quit()
		return
